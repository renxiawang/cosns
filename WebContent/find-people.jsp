...
<%
	...
	List<User> resultList = (LinkedList<User>) session.getAttribute("UserList");
%>
		...
		<div class="main-middle">
			...
			if (resultList!=null) {
				for (int i = 0; i < resultList.size(); i++) { %>
					...
					<img class="news-avatar" style="margin-top:5px;" src=<%=resultList.get(i).getAvatarUrl() %> />
					<div style="margin:-10px 0 0 100px;">
						<div style="float:left;width:200px;"><h3><a href=<%="profile.jsp?uid=" + resultList.get(i).getUID() %>><%=resultList.get(i).getFullName() %></a></h3></div>
						<div style="float:right;"><a href=<%="FollowSVL?uid=" + resultList.get(i).getUID() %>><img style="margin-right:50px" src="images/follow.png" /></a>	</div>
					</div>
				</div>
				<%}
			}
